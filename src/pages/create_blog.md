...
title: "纯静态博客完全自动化部署：基于 Webhook + CF Tunnel 的极客实践"
date: "2026-03-19"
---
...
services:
  webhook:
    ...
    volumes:
      ...
      # 映射博客项目，让容器有权限执行脚本
      - /root/blog:/root/blog
      # 映射 Docker 进程，允许控制宿主机 Docker
      - /var/run/docker.sock:/var/run/docker.sock
      # 借用宿主机的 SSH 密钥用于 Git 拉取 (只读)
      - /root/.ssh:/root/.ssh:ro
    command: -verbose -hooks=/etc/webhook/hooks.json -hotreload
```

### 3.3 触发器规则 hooks.json
在 `/root/webhook/` 目录下创建，告诉 Webhook 收到请求后去哪里执行什么：
...
放在博客根目录下 (`/root/blog/deploy.sh`)，稳如磐石的执行逻辑：
...
现在，我只需要在本地炫酷的 `/console` 面板调好星空粒子特效的参数，在终端优雅地敲下 `git push`，剩下的拉取、打包、构建、重启，全在十几秒内由远端 GCP 服务器默默完成。
